...
3. Copy `assets/intent.md` into `intent/<slug>/intent.md`. Fill every section, including Evidence. Frontmatter: `status: draft`, `slug: <slug>`. Do not migrate existing consumer intents that lack Evidence. Remove from `context.md` anything now captured in `intent.md`; remove the file if nothing remains. Commit these edits before the next step.
