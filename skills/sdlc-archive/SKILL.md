...
description: >-
  ...
  and verdict: pass with no CRITICAL. Fail or CRITICAL → apply must fix then
  re-verify first; do not archive, do not ask to skip.
license: MIT
metadata:
  author: acourtiol
  version: "1.2"
...
5. **Hard stop without a passing verify.** If `report.md` is missing: stop; next is `sdlc-verify`. If `verdict` is not `pass` or Findings still has a CRITICAL item: stop; next is `sdlc-apply` (fix) then `sdlc-verify`. Do not archive. Do not ask to skip. Other gaps (statuses not `done`, unticked plan boxes as `N/M`) are warnings: say what is short, ask, and archive only if they confirm those — never if verify is missing, failing, or still CRITICAL.
